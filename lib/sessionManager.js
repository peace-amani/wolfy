...
export async function getAllSessions() {
  try {
    ...
    return dbSessions.map(s => ({
      phone: s.phone,
      status: activeSessions.has(s.phone) ? 'active' : s.status,
...
export async function getSessionStats() {
  try {
    ...
    const active = all.filter(s => activeSessions.has(s.phone)).length;
    ...
    return { total: 0, active: 0, inactive: 0, pairing: 0, inMemory: activeSessions.size };
  }
}
